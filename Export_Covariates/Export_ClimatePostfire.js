...
var Daymet = ee.ImageCollection("NASA/ORNL/DAYMET_V3"),
    ...
    // Process fire polygons by fire frequency
...
// For exporting tables use these parameters
var fire = 'WEST BASIN'
var name = 'WestBasin'
var pts_fire = fire_pts.filterMetadata('Fire_Name', 'equals', fire)
var fire_geom = fires.filterMetadata('Fire_Name', 'equals', fire)

// For exporting images for predictive mapping use these parameters
var fire_geom = SaddleDraw
var fire_year = ee.Number(fire_geom.first().get('Year'))

// Bounding box to clip by
// var GBbounds_f = ee.Feature(GBbounds.first()) // Use this for exporting tables
var GBbb = fire_geom.geometry() // Use this for exporting images for predictive mapping


// ---------------------------- Add preliminary bands ----------------------------
...
// ---------------- Define temporal parameters in relation to the fire ----------------

// Filter to 30 year period
var startDate = ee.String(fire_year.subtract(1)).cat('-01-01')
var endDate = ee.String(fire_year.add(3)).cat('-01-01')
...
var startYear = fire_year;
var endYear = fire_year.add(3);
...
// Spring filter
var filterSpr = ee.Filter.calendarRange(4, 5, 'month')

// Summer filter
var filterSum = ee.Filter.calendarRange(7, 8, 'month')

// Winter filter
var filterWin = ee.Filter.calendarRange(11, 2, 'month')

// Growing season filter
var filterGrS = ee.Filter.calendarRange(4, 9, 'month')
...
var SumMaxTCol = ee.ImageCollection(yearList.map(function(year){
  ...
  var startdate = '-06-01' // Start in June
  var enddate = '-10-01'// End in October
...
// Reduce Spring and Autumn frost image collections to produce the post-fire mean image
var SprF = SprFCol.reduce(ee.Reducer.mean()).rename('SprF')
var AutF = AutFCol.reduce(ee.Reducer.mean()).rename('AutF')
// Calculate mean frost free period to produce the post-fire mean image
...
// ----------- Calculate mean maximum snow water equivalent (SWE) -----------
// Reducer for image collection to annual maximum
...
var CliMeans_list = ee.List([MAP, SprP, SumP, GrSP, WinP, MT, SprT, SumT, GrST, WinT, SumWinDiff, SumMaxT, WinMinT, SprF, AutF, FFree, SWE,DD0Min,DD5ff, DD5, adi, sdi]) // DD0Min,DD5ff, DD5, adi, sdi
var CliMeans_IC = ee.ImageCollection(CliMeans_list)
var CliMeans_I = CliMeans_IC.toBands().clip(fire_geom)
print(CliMeans_I)
Map.addLayer(CliMeans_I.select('0_MAP'))

// Run reduce regions on image
var Postfire_rr = CliMeans_I.reduceRegions({
  collection: pts_fire,
...
Export.table.toDrive({
    collection: Postfire_rr,
    description: name + '_cli_rr',
    fileFormat: 'csv'}) 

// Create single image to export by
var CliMeans_I = ee.ImageCollection([MAP,SWE,sdi,SumP])
                        .toBands()
                        .clip(GBbb)
...
for (var i = 0; i < 4; i++) {
  // Get image
  var img = CliMeans_I_resample.select(namelist[i]).clip(GBbounds.geometry());
  
  // Export results to drive
  Export.image.toDrive({
      image: img,
      description: namelist[i].substr(2,10) +'_pst',
      scale: 30,
      maxPixels: 1e13,
      region: GBbb});}  
