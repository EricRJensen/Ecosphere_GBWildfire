...
# Topographic
setwd('C:/Users/erjensen/Documents/Thesis/ThesisAIM/data/Recov_CSVs/New/')
...
for(i in clia_files){
  ...
  print(var_str)
  name = var_str
  indv_cli_df <- read_csv(i,na=c("","NA")) %>% dplyr::select(c(ID, contains('mean')))
  indv_cli_df[, paste(name, '_avg', sep = '')] <- indv_cli_df$mean
  indv_cli_df <- dplyr::select(indv_cli_df,-mean)
  clia_vars <- left_join(clia_vars, indv_cli_df, by = 'ID') %>% unique() }
...
Post_train <- Post_train %>% dplyr::select(-c(p.Year, p.latitude, p.longitude, p.Fire_Name, vars_mc))
Post_valid <- Post_valid %>% dplyr::select(-c(p.Year, p.latitude, p.longitude, p.Fire_Name, vars_mc))
...
Post_train <- Post_train %>% dplyr::select(c(r.NDPDI_post, Post_sel$parameters[[4]]))
...
# Produce figures of validation between predictions and observed
png("C:/Users/erjensen/Desktop/PredObs.png", units = 'in', width = 4, height = 4, type = 'cairo', res = 400)
ggplot(rf_val_preds, aes(x=Response, y = Predicted)) +
  ...
  labs(x = "15-year post-fire NDPDI", y = "Modeled predictions of 15-year post-fire NDPDI")+# title = "Modeling Contemporary NDPDI from RAP") +
  #annotation_custom(rf_rsq_grob) +  annotation_custom(rf_rmse_grob) +
  scale_x_continuous(limits = c(0,2), breaks = seq(0,2,.4)) +
  scale_y_continuous(limits = c(0,2), breaks = seq(0,2,.4)) +
  theme_minimal()
dev.off()

png("C:/Users/erjensen/Desktop/SumP_pst.png", units = 'in', width = 4, height = 4, type = 'cairo', res = 400)
ggplot(Post_train, mapping = aes(x = as.factor(p.TimesBurn), y = r.NDPDI_post))+
  geom_boxplot(alpha = .3, shape = 16)+
  labs(x = 'Times burned', y = '15-year-post-fire NDPDI')+
  theme_minimal() +
  theme(legend.position = "none")
dev.off()

seed_df = Post_train %>%
  dplyr::select(c(r.NDPDI_post, p.Drill, p.Aerial)) %>%
  mutate(p.Drill = ifelse(p.Drill ==1 , 2, 0),
         p.Seed = as.factor(as.numeric(p.Drill) + as.numeric(p.Aerial)),
         p.Seed = fct_recode(p.Seed, Unseeded = '1', Aerial = '2', Drill = '3', Both = '4'))

ggplot(seed_df, mapping = aes(x = as.factor(p.Seed), y = r.NDPDI_post))+
  geom_boxplot(alpha = .3, shape = 16)+
  labs(x = 'Seeding', y = '15-year-post-fire NDPDI')+
  theme_minimal() +
  theme(legend.position = "none")


#------------------------ Apply predictive model to Saddle Draw Fire --------------------------------

# Apply predictive model for 15 year post-fire condition
tifs <- list.files('C:/Users/erjensen/Documents/Thesis/ThesisAIM/data/Recov_rasters/SaddleDraw/', pattern='tif', full.names = TRUE)
ModelRasters = stack(tifs)

# Predict the model using the predictor rasters
...
plot(SaddlePost)
plot(Saddle)
writeRaster(Saddle, 'C:/Users/erjensen/Documents/Thesis/ThesisAIM/data/SaddleNDPDI_Predpost.tif')
